...
public interface AlarmRepository extends JpaRepository<AlarmEntity, Integer> {
    ...
    @Query(value = "SELECT * FROM alarm where user_id = :id and reference_id = :referenceId and alarm_type = :alarmType", nativeQuery = true)
    AlarmEntity findByUserIdAndReferenceId(@Param("id") String id, @Param("referenceId") String referenceId, @Param("alarmType")AlarmType alarmType);
}
