...
public class AuthServiceImplement implements AuthService {
    ...
    public ResponseEntity<? super SignInResponseDto> signIn(SignInRequestDto requestDto) {
        try {
            if (requestDto == null) return ResponseDto.badRequest();
            UserEntity userEntity = userRepository.findById(requestDto.getId()).orElse(null);
            if (userEntity == null) return SignInResponseDto.notExsitedId();
            if (!passwordEncoder.matches(requestDto.getPassword(), userEntity.getPassword())) return SignInResponseDto.notExsitedPassword();
    ...
    public ResponseEntity<? super SignUpResponseDto> signUp(SignUpRequestDto requestDto) {
        try {
            if (requestDto == null) return ResponseDto.badRequest();
            if (!requestDto.isDuplicateCheckId()) return SignUpResponseDto.duplicateIdNotCheck(); 
            ...
            if (!IsVerify) return SignUpResponseDto.notVerifyCode();

            UserEntity userEntity = new UserEntity(requestDto.getId(), passwordEncoder.encode(requestDto.getPassword()), requestDto.getName(), requestDto.getSex(), "", requestDto.getEmail(), requestDto.getAddress(), requestDto.getAddressDetail(), "");
    ...
    public ResponseEntity<? super SignInResponseDto> oauth(Map<String, Object> userInfo) {
        try {
            if (userInfo == null) SignInResponseDto.badRequest();
            ...
            UserEntity userEntity = new UserEntity(id, null, name, null, null, email, null, null, profileImage);
            userRepository.save(userEntity);
    ...
    public ResponseEntity<? super DuplicateCheckIdResponseDto> duplicateCheckId(DuplicateCheckIdRequestDto requestDto) {
        try {
            ...
            if (exist) return DuplicateCheckIdResponseDto.faild();
